import './utils.js'
import { Command, Room, serializeCommand } from './room.js'
import initMenu from './menu.js'
import { SliderEvent } from './menu.js'
import { Grid, Mark, CellCoords, Guess, RatedGuess, Options, OptionsMenu } from './defs'

document.addEventListener('DOMContentLoaded', async () => {
	const table = document.getElementById('table') as HTMLDivElement
	const curPlayerDiv = document.getElementById('cur-player') as HTMLDivElement

	const gridDiv = document.getElementById('grid') as HTMLDivElement

	initMenu()
	const menu = document.querySelector<OptionsMenu>('.menu')
	...
	const restartTitle = restartPage.children[0] as HTMLDivElement
	const [restartButton, exitButton] = [...restartPage.children[1].children] as HTMLDivElement[]
	const restartReadycheck = restartPage.children[2] as HTMLDivElement
	...
	const c = (m: Mark) => (m === 1 ? 2 : 1) as Mark
	...
	const checkTurnValidity = (cellCoords: CellCoords) => {
		const [x, y] = cellCoords
		if (grid.length <= y) return false
		if (grid[0].length <= x) return false
	...
	const isWinningTurn = (grid: Grid, turn: CellCoords, mark: Mark) => {
		...
		let [l, r, t, b, lt, rt, rb, lb] =
			[0, 0, 0, 0, 0, 0, 0, 0]

		for (let i = 1; i <= tL; i++) { if (g[y][x - i] !== m) break; l++ }
		for (let i = 1; i <= tR; i++) { if (g[y][x + i] !== m) break; r++ }
		for (let i = 1; i <= tT; i++) { if (g[y - i][x] !== m) break; t++ }
		for (let i = 1; i <= tB; i++) { if (g[y + i][x] !== m) break; b++ }

		for (let i = 1; i <= Math.min(tL, tT); i++) { if (g[y - i][x - i] !== m) break; lt++ }
		for (let i = 1; i <= Math.min(tR, tT); i++) { if (g[y - i][x + i] !== m) break; rt++ }
		for (let i = 1; i <= Math.min(tL, tB); i++) { if (g[y + i][x - i] !== m) break; lb++ }
		for (let i = 1; i <= Math.min(tR, tB); i++) { if (g[y + i][x + i] !== m) break; rb++ }

		const d = (a: number, b: number) => a + b + 1
	...
	const checkForWinner = (lastTurn: CellCoords, lastMark: Mark): 0 | 1 | 2 => {
		if (isWinningTurn(grid, lastTurn, lastMark)) return 1

		if (grid.some(row => row.some(cell => cell === 0))) return 0
	...
	type Unit = {
		...
		update(turn: CellCoords, mark: Mark): void
	}

	class LocalPlayer implements Unit {
		name: string
			...
			while (true) {
				turn = await new Promise(res => {
					...
					divGrid.forEach((row, y) => {
						row.forEach((div, x) => {
							if (!grid[y][x]) {
								div.addEventListener('pointerdown', (pe: PointerEvent) => {
									controller.abort()

									const i = divGrid.flat().indexOf(pe.target as HTMLDivElement)
				...
				if (checkTurnValidity(turn)) break

				console.warn('Invalid placement')
			}

			return turn as CellCoords
		}

		update(turn: CellCoords, mark: Mark) {
			const [x, y] = turn

			if (grid[y][x] && mark) throw new Error('Cell is occupied', { cause: `[${turn}]` })
			...
			else classList.remove('circle', 'cross')
		}
	}

	class AIPlayer implements Unit {
		#solve: () => void

		#armSolver() {
			this.#solved = new Promise<void>(res => { this.#solve = res })
		}

		#solved = null as Promise<void>
		...
		_guessTree: Guess[]
		_lastEnemyTurn: CellCoords
		_currentBranch: Guess[]

		constructor(name: string, mark: Mark, firstMark: Mark, difficulty: number) {
			this.name = name
			this.mark = mark
		...
		async makeTurn() {
			...
			if (checkTurnValidity(turn.c)) return turn.c

			throw new Error('Bot calculated invalid turn')
		}

		update(turn: CellCoords) {
		...
		async calc() {
			...
			this._guessTree = this._currentBranch = AIPlayer.makeResponses(
				grid,
				this.firstMark === this.mark,
				this.mark,
				this.difficulty
			)
		...
		static makeResponses(grid: Grid, botsTurn: boolean, botsMark: Mark, difficulty: number) {
			const m = botsMark

			if (botsTurn) {
				//* Bot turn
				const botChoices = [] as RatedGuess[]
				...
				if (!botChoices.length) return null

				let bestTurns = [botChoices[0]] as RatedGuess[]
				const obligatory = [] as RatedGuess[]
				...
				for (let i = 1; i < botChoices.length; i++) {
					...
					if (guess.lr < lr) bestTurns = [guess]
					else if (guess.lr === lr) {
						if (guess.wr > wr) bestTurns = [guess]
						else if (guess.wr === wr) bestTurns.push(guess)
					}
				}
				...
				//* Opponent turn
				const oppChoices = [] as RatedGuess[]
	...
	class RemotePlayer implements Unit {
		#listeners = [] as [string, (c: Command) => void][]
		#messageStack = [] as Command[]

		#restartVoted: Promise<void>
		#armRestartVote() {
			let trigger: () => void
			this.#restartVoted = new Promise<void>(res => {
				trigger = res
			})
		...
		init = () => new Promise<void>(res => {
			...
			if (this.isHost) {
				...
				clearGrid()
			} else this.once('start', c => {
				const a = c.args

				if (a?.length < 4) throw this.#err('start', c)
		...
		makeTurn = () => new Promise<CellCoords>(res => {
			this.once('turn', c => {
				const a = c.args

				if (a?.length < 2) throw this.#err('turn', c)

				const
					x = +a[0],
					y = +a[1]

				if (!checkTurnValidity([x, y])) throw this.#err('turn', c)
		...
		once(name: string, cb: (c: Command) => void) {
		...
		get isHost() {
			if (this._room.state < 3) throw new Error('Room is not connected')
	...
	const runGame = async () => {
		...
		if (!(player2 instanceof RemotePlayer)) clearGrid()
		...
		const player = (main = true): Unit => playerXTurn === main ? player1 : player2
		const m = (): Mark => playerXTurn === firstTurn ? 1 : 2

		gameCycle: while (true) {
			const p = player()
			const cp = player(false)

			if (p === player1) curPlayerDiv.classList.remove('second')
			else curPlayerDiv.classList.add('second')
			curPlayerDiv.innerText = `${p.name}'s turn`

			console.log(`${p.name}'s turn`)

			const turn = await p.makeTurn()
			if (p == player1 && !(player2 instanceof LocalPlayer)) player1.update(turn, m())
			...
			switch (checkForWinner(turn, m())) {
				case 1:
					...
					if (player() === player1) restartTitle.classList.remove('second')
					else restartTitle.classList.add('second')
		...
		const resGame = () => {
			...
			firstTurn = !firstTurn
			if (player2 instanceof AIPlayer) player2.swapSides()
...

	{
		...
		if (urlParams.has('animod')) table.classList.add('animod')
		...
		settingsPage.options[0].addEventListener('change', (e: SliderEvent) => updateDisables(e.value))
		...
		const mainListener = () => {
			...
			if (+settings[2][0] < 3 || +settings[2][0] > 7) {
				...
				return
			} else gridX.classList.remove('fail')

			if (+settings[2][1] < 3 || +settings[2][1] > 7) {
				gridY.classList.add('fail')
				return
			} else gridY.classList.remove('fail')
			...
			switch (options.p2) {
				...
				case 'net':
					...
					const [roomMenuHost, roomMenuJoin] =
						roomPage.querySelectorAll<HTMLDivElement>('.button')
					const [upperText, lowerText] =
						messagePage.querySelectorAll<HTMLDivElement>('.text')
					...
					const updateLoadingMessage = (error: boolean, upper: string, lower?: string) => {
						...
						} else lowerText.style.display = 'none'
					}
					...
					const roomJoinListener = () => {
						...
						if (!/^[a-z0-9]{6}$/.test(roomID)) return reject()
